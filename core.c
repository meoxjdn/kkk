 ...
 * =====================================================================================
 * Filename:  core.c
 * Description:  Ghost Core Engine V27.6 (Kill67 Synced + Yield Bug Fixed)
 * Architecture:  AArch64 (ARMv8-A + PAC Aware + Full CFI Immune)
 * Status:  Production Ready (Global Array Protected / Logic Aligned)
 * =====================================================================================
 */
...
static uint64_t find_lib_base_kernel(struct task_struct *task, const char *lib_name) {
    ...
    for (vma = mm->mmap; vma; vma = vma->vm_next) {
        ...
        if (vma->vm_file && vma->vm_file->f_path.dentry) {
            if (strstr(vma->vm_file->f_path.dentry->d_name.name, lib_name)) { addr = (uint64_t)vma->vm_start; break; }
        }
    }
    mmap_read_unlock(mm); mmput(mm); return addr;
}
...
static int ghost_read_task_mem(struct task_struct *task, unsigned long uaddr, void *dest, size_t size) {
    struct mm_struct *mm; pgd_t *pgd; p4d_t *p4d; pud_t *pud; pmd_t *pmd; pte_t *pte;
    unsigned long pa, pmd_phys; int ret = 0; struct page *page; void *kmap_addr;

    mm = get_task_mm(task); if (!mm) return -ESRCH;
    mmap_read_lock(mm);
...
out_unlock: mmap_read_unlock(mm); mmput(mm); return ret;
}
...
__nocfi static void wuwa_hbp_handler(struct perf_event *bp, struct perf_sample_data *data, struct pt_regs *regs) {
    ...
    if (l_base != 0 && pc == l_base + HIJACK_OFFSET) { regs->pc = ptrauth_strip_insn_pac(regs->regs[30]); return; }

    /* =========================================================
     * 核心修复：执行真实的 67 强杀拦截逻辑，欺骗状态机！
     * ========================================================= */
    if (g_cfg.kill67_on && base != 0 && pc == base + g_cfg.off_kill67) {
        regs->regs[0] = 1;      // 修改 X0 寄存器，伪造校验返回值
        regs->pc = ptrauth_strip_insn_pac(regs->regs[30]); // 跳出函数，让原校验闭嘴
        return;
    }

    if (g_cfg.border_on && pc == base + g_cfg.off_border) { regs->regs[0] = 0; regs->pc = ptrauth_strip_insn_pac(regs->regs[30]); return; }
    ...
    if (g_cfg.damage_on && pc == base + g_cfg.off_damage) {
        uint64_t target_addr = regs->regs[1] + 0x1C; uint32_t flag_val = 0;
    ...
    if (g_cfg.maxhp_on && pc == base + g_cfg.off_kill) { regs->regs[0] = 1; regs->pc = ptrauth_strip_insn_pac(regs->regs[30]); return; }

    if (g_cfg.fov_on && pc == base + g_cfg.off_fov) { 
        if (g_cfg.fov_is_ptr && g_cfg.fov_reg >= 0 && g_cfg.fov_reg <= 30) regs->regs[g_cfg.fov_reg] = base + g_cfg.fov_val; 
...
__nocfi static struct perf_event *install_bp(struct task_struct *tsk, uint64_t addr, perf_overflow_handler_t handler) {
    ...
    hw_breakpoint_init(&attr); attr.bp_addr = addr; attr.bp_len = HW_BREAKPOINT_LEN_4; attr.bp_type = HW_BREAKPOINT_X; attr.disabled = 0;
    bp = fn_register(&attr, handler, NULL, tsk); return IS_ERR(bp) ? NULL : bp;
}
    ...
    if (attr_uptr && fn_copy_nofault && fn_copy_to_user_nofault) {
        if (fn_copy_nofault(&stash->attr, attr_uptr, sizeof(struct perf_event_attr)) == 0) {
            if (stash->attr.type == PERF_TYPE_BREAKPOINT) {
                uint32_t fake_type = PERF_TYPE_SOFTWARE; uint64_t fake_config = PERF_COUNT_SW_DUMMY;
                ...
                stash->is_fake_target = true; stash->uptr = attr_uptr;
            }
        }
    }
    return 0;
...
__nocfi static int entry_handler_ptrace(struct kretprobe_instance *ri, struct pt_regs *regs) {
    ...
    stash->request = regs->regs[0]; stash->target_pid = regs->regs[1]; stash->data = (void __user *)regs->regs[3]; 
    stash->is_fake_target = false;
    ...
    if (stash->is_fake_target && fn_copy_nofault) {
        if (fn_copy_nofault(&stash->iov, stash->data, sizeof(struct iovec)) == 0) regs->regs[3] = 0; else stash->is_fake_target = false;
    }
    return 0;
}
...
static void ptrace_yield_worker(struct work_struct *w) {
    ...
    int ac_active = 0, i; struct task_struct *tsk; struct pid *pid_struct;

    for (i = 0; i < ARM64_MAX_HW_BPS; i++) if (pw->incoming.dbg_regs[i].ctrl & 1) ac_active++;

    pid_struct = find_get_pid(pw->target_pid); if (!pid_struct) { kfree(pw); return; }
    tsk = pid_task(pid_struct, PIDTYPE_PID); if (!tsk) { put_pid(pid_struct); kfree(pw); return; }

    mutex_lock(&g_bp_mutex);
    
    /* =========================================================
     * 核心修复：修正退让算法 Bug！只算单个线程的断点占用。
     * 防止 g_bp_count(总和57) > 6 导致永远无法生效的错误！
     * ========================================================= */
    int my_active_bps_per_thread = 0;
    if (g_cfg.kill67_on) my_active_bps_per_thread++;
    if (g_cfg.border_on) my_active_bps_per_thread++;
    if (g_cfg.skip_on)   my_active_bps_per_thread++;
    if (g_cfg.damage_on) my_active_bps_per_thread++;
    if (g_cfg.fov_on)    my_active_bps_per_thread++;
    if (g_cfg.maxhp_on)  my_active_bps_per_thread++;

    if (g_bp_count > 0 && (my_active_bps_per_thread + ac_active > ARM64_MAX_HW_BPS)) {
        for (i = 0; i < g_bp_count; i++) {
            if (g_bps[i]) { perf_event_disable(g_bps[i]); if (fn_unregister) fn_unregister(g_bps[i]); g_bps[i] = NULL; }
        }
        g_bp_count = 0; g_yielded_flag = 1;
    }
        ...
        if (g_cfg.kill67_on && g_game_base && g_bp_count < MAX_BPS) { bp = install_bp(tsk, g_game_base + g_cfg.off_kill67, wuwa_hbp_handler); if (bp) g_bps[g_bp_count++] = bp; }
    ...
    for (i = 0; i < ARM64_MAX_HW_BPS; i++) {
        int enabled = pw->incoming.dbg_regs[i].ctrl & 1; uint64_t ac_addr = pw->incoming.dbg_regs[i].addr;
        if (enabled) {
            ...
            if (g_ac_bps[i]) { if (fn_unregister) fn_unregister(g_ac_bps[i]); g_ac_bps[i] = NULL; }
        }
    }
    mutex_unlock(&g_bp_mutex); put_pid(pid_struct); kfree(pw);
}

__nocfi static int ret_handler_ptrace(struct kretprobe_instance *ri, struct pt_regs *regs) {
    struct ptrace_stash *stash = (struct ptrace_stash *)ri->data; long ret = regs_return_value(regs);
    if (stash->is_fake_target && (ret == -EFAULT || ret == -ENOSPC || ret == 0)) {
        ...
        if (stash->request == PTRACE_SETREGSET && stash->iov.iov_len <= sizeof(*target)) {
            ...
            if (pw) {
                pw->target_pid = stash->target_pid; pw->target_ledger = stash->target_ledger;
                if (fn_copy_nofault(&pw->incoming, stash->iov.iov_base, stash->iov.iov_len) == 0) {
                    memcpy(target, &pw->incoming, stash->iov.iov_len); INIT_WORK(&pw->work, ptrace_yield_worker); schedule_work(&pw->work);
                } else kfree(pw);
            }
            regs->regs[0] = 0; 
        } else if (stash->request == PTRACE_GETREGSET) {
            if (fn_copy_to_user_nofault) fn_copy_to_user_nofault(stash->iov.iov_base, target, min_t(size_t, stash->iov.iov_len, sizeof(*target)));
            regs->regs[0] = 0;
        }
    }
    return 0;
...
__nocfi int wuwa_install_perf_hbp(struct wuwa_hbp_req *req) {
    ...
    if (g_bp_count == 0 && !g_yielded_flag) {
        g_target_tgid = tsk->tgid; WRITE_ONCE(g_game_base, req->base_addr);
        uint64_t l_base = find_lib_base_kernel(tsk, TARGET_LIB_NAME);
        if (l_base != 0) { WRITE_ONCE(g_lib_base, l_base); if (g_bp_count < MAX_BPS) { bp = install_bp(tsk, l_base + HIJACK_OFFSET, wuwa_hbp_handler); if (bp) g_bps[g_bp_count++] = bp; } }
    }
    
    memcpy(&g_cfg, req, sizeof(struct wuwa_hbp_req)); smp_mb(); 
    
    if (!g_yielded_flag) {
        if (req->kill67_on && g_game_base != 0 && g_bp_count < MAX_BPS) { bp = install_bp(tsk, g_game_base + req->off_kill67, wuwa_hbp_handler); if (bp) g_bps[g_bp_count++] = bp; }
    ...
    mutex_unlock(&g_bp_mutex); put_pid(pid_struct); return 0;
}
    ...
    if (pid_struct) {
        ...
        if (tsk && g_target_tgid != 0 && tsk->tgid == g_target_tgid) {
            ...
            if (!g_yielded_flag && g_bp_count < (MAX_BPS - 10)) {
                ...
                if (g_cfg.kill67_on && g_game_base) { bp = install_bp(tsk, g_game_base + g_cfg.off_kill67, wuwa_hbp_handler); if (bp) g_bps[g_bp_count++] = bp; }
...
static int __init ghost_core_init(void) {
    ...
    for (i = 0; i < 4; i++) { wuwa_nl_sk = netlink_kernel_create(&init_net, ports[i], &nl_cfg); if (wuwa_nl_sk) break; }
    if (!wuwa_nl_sk) return -ENOMEM;

    krp_perf = (struct kretprobe){ .entry_handler = entry_handler_perf, .handler = ret_handler_perf, .data_size = sizeof(struct perf_stash), .maxactive = 64, .kp.symbol_name = "__arm64_sys_perf_event_open" };
    register_kretprobe(&krp_perf);
    krp_ptrace = (struct kretprobe){ .entry_handler = entry_handler_ptrace, .handler = ret_handler_ptrace, .data_size = sizeof(struct ptrace_stash), .maxactive = 64, .kp.symbol_name = "__arm64_sys_ptrace" };
    register_kretprobe(&krp_ptrace);
    krp_clone = (struct kretprobe){ .handler = clone_ret_handler, .maxactive = 128, .kp.symbol_name = "__arm64_sys_clone" };
    register_kretprobe(&krp_clone);
    cloak_module(); return 0;
}

static void __exit ghost_core_exit(void) {
    unregister_kretprobe(&krp_perf); unregister_kretprobe(&krp_ptrace); unregister_kretprobe(&krp_clone);
