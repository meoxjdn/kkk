...
static void wuwa_hbp_handler(struct perf_event *bp, struct perf_sample_data *data, struct pt_regs *regs) {
    uint64_t pc; 
    uint64_t base;
    /* 加上 __attribute__((unused)) 防止编译报 warning 变 error */
    ...
    if (g_cfg.damage_on && pc == base + g_cfg.off_damage) {
        ...
        /* 默认分支：如果是玩家攻击，或者进图时读取失败，老老实实放行。
         * 手动替系统执行被断点覆盖掉的新版第一条汇编：SUB SP, SP, #0x40 */
        regs->sp -= 0x40; 
        regs->pc += 4; 
        return;
    }
