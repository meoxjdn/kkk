...
static unsigned long p_reg_step = 0; static unsigned long p_unreg_step = 0;
static unsigned long p_en_step = 0;  static unsigned long p_dis_step = 0;
static unsigned long p_fp_pres = 0;  static unsigned long p_fp_upd = 0;
static unsigned long p_reg_brk = 0;  static unsigned long p_unreg_brk = 0;
...
static long wuwa_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    else if (cmd == WUWA_IOCTL_CLEAN_ALL) {
        clear_all_shadows();
        return 0;
    }
    return -EINVAL;
...
static int __init wuwa_driver_init(void)
{
    ...
    int ret;
    syms.p_register_user_step_hook = p_reg_step;       syms.p_unregister_user_step_hook = p_unreg_step;
    syms.p_user_enable_single_step = p_en_step;        syms.p_user_disable_single_step = p_dis_step;
    syms.p_fpsimd_preserve_current_state = p_fp_pres;  syms.p_fpsimd_update_current_state = p_fp_upd;
    syms.p_register_user_break_hook = p_reg_brk;       syms.p_unregister_user_break_hook = p_unreg_brk;

    ret = shadow_fault_init_dynamic(&syms);
    if (ret < 0) return ret;
    
    return misc_register(&wuwa_misc);
}
