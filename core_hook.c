 ...
 *    Description:  Ghost Core V10.13 (Hybrid Precision & FPU Safe Edition)
 *   Architecture:  AArch64 (ARMv8-A)
 *         Status:  Ultimate Production Ready (Full Implementation)
 *         Author:  顶尖逆向架构师
 * 
 * =====================================================================================
 */
...
/* FPU 浮点上下文接管指针，用于安全修改 NEON/SIMD 寄存器 */
...
struct hwbp_thread_node {
    ...
    int sm_state;        
    uint64_t saved_lr;   
    /* 完全复用宿主内核 <uapi/asm/ptrace.h> 的结构体，保证 ABI 级兼容 */
...
/* ==========================================================
 * 模块二：混合精度控制流路由 (The Hybrid Router)
 * 逆向解构：融合 CFG 即时远跳与单断点栈帧平衡状态机
 * ========================================================== */
static void ghost_hwbp_handler(struct perf_event *bp, struct perf_sample_data *data, struct pt_regs *regs)
{
    ...
    list_for_each_entry_rcu(node, &hwbp_thread_list, list) {
        if (node->bp_event == bp) {
            ...
            /* 
             * 策略分支 A：即时控制流重定向 (Immediate CFG Redirection)
             * 直接修改 PC，跳过函数极序 (Prologue)，适用于远跳及手动栈平衡。
             */
            if (node->func_id == 2) {
                /* FID 2: 秒过。直接飞入目标逻辑链，绕过原判定 */
            ...
            if (node->func_id == 4) {
                /* FID 4: 伤害/无敌。引入内存探针校验与手动 Unwind */
                uint32_t flag = 0;
                ...
                /* 未命中判定：手动解开 0x30 的栈帧并安全退出，保障调用约定 */
                regs->sp += 0x30;
                regs->regs[0] = 1;
            ...
            /* 
             * 策略分支 B：完美栈帧状态机 (State-Machine Call Interception)
             * 原理：入口打卡 -> LR等候 -> 截获返回值。保证 Prologue/Epilogue 闭环。
             */
            if (p_modify_hwbp) {
                new_attr = bp->attr;
                ...
                if (node->sm_state == 0) {
                    ...
                    /* 阶段 2：命中 LR 出口。此时栈帧已由原函数平衡完毕 */
                    if (node->func_id == 1 || node->func_id == 3) {
                        /* FID 1(去黑边) / FID 3(秒杀): 写入布尔真值，防止越界崩溃 */
                        regs->regs[0] = 1; 
                    }
                    else if (node->func_id == 5) {
                        /* 
                         * FID 5(全屏FOV): 浮点上下文(FPR)软着陆。
                         * 使用内核原生导出函数安全读写 current 线程的 fp 状态，杜绝上下文污染。
                         */
                        if (p_fpsimd_save && p_fpsimd_load) {
                            struct user_fpsimd_state *fp = &current->thread.uw.fpsimd_state;
                            p_fpsimd_save(fp);
                            fp->vregs[0] = (fp->vregs[0] & ~((__uint128_t)0xFFFFFFFFULL)) | (__uint128_t)0x42F00000;
                            p_fpsimd_load(fp);
                        }
                    }
...
static void task_work_elastic_hwbp(struct callback_head *work)
{
    ...
    install_hwbp_on_thread(current, en->tgid, en->target_addr, en->func_id, en->aux_addr);
...
static void wq_elastic_hwbp(struct work_struct *work)
{
    ...
    install_hwbp_on_thread(en->task, en->tgid, en->target_addr, en->func_id, en->aux_addr);
...
static int handler_pre_wake_up_new_task(struct kprobe *p, struct pt_regs *regs)
{
    ...
    list_for_each_entry_rcu(tgt, &hwbp_target_list, list) {
        if (tgt->tgid == new_task->tgid) {
            ...
            if (en) {
                en->tgid = tgt->tgid;
                en->target_addr = tgt->addr;
                en->func_id = tgt->func_id;
                en->aux_addr = tgt->aux_addr;
                en->task = new_task;
...
long handle_hwbp_ioctl(unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        case IOCTL_SET_HWBP:
            ...
            tgt->tgid = req.tgid;
            tgt->addr = req.target_addr;
            tgt->func_id = req.function_id;
            tgt->aux_addr = req.aux_addr;
            ...
            for (i = 0; i < count; i++) {
                install_hwbp_on_thread(tasks[i], req.tgid, req.target_addr, req.function_id, req.aux_addr);
        ...
        case IOCTL_PAUSE_HWBP:
            rcu_read_lock();
            list_for_each_entry_rcu(node, &hwbp_thread_list, list) {
                if (node->tid == current->pid && node->bp_event) {
                    perf_event_disable(node->bp_event);
                    break;
                }
            }
            rcu_read_unlock();
        ...
        case IOCTL_RESUME_HWBP:
            ...
            list_for_each_entry_rcu(node, &hwbp_thread_list, list) {
                if (node->tid == current->pid && node->bp_event) {
                    perf_event_enable(node->bp_event);
                    break;
                }
            }
            rcu_read_unlock();
...
int ghost_core_init_engine(void)
{
    ...
    pr_info("[GhostCore V10.13] Hybrid CFG Router & FPU Secure Injector Online.\n");
    return 0;
}
...
void ghost_core_exit_engine(void)
{
    ...
    spin_lock_bh(&hwbp_thread_lock); 
    list_splice_init(&hwbp_thread_list, &local_hwbp); 
    spin_unlock_bh(&hwbp_thread_lock);
    
    spin_lock_bh(&hwbp_target_lock); 
    list_splice_init(&hwbp_target_list, &local_target); 
    spin_unlock_bh(&hwbp_target_lock);

    synchronize_rcu();

    list_for_each_entry_safe(node, ntmp, &local_hwbp, list) {
        if (node->bp_event && p_unregister_hwbp) {
            perf_event_disable(node->bp_event);
            p_unregister_hwbp(node->bp_event);
        }
        kfree(node);
    }
    ...
    pr_info("[GhostCore V10.13] Resources safely drained. Teardown Complete.\n");
}
