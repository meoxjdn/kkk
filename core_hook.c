    ...
    list_for_each_entry(entry, &shadow_page_list, list) {
        ...
        case 1: {
            ...
            if (!(new_rot[0] == 0 && new_rot[1] == 0 && new_rot[2] == 0)) {
                ...
                preempt_disable();
                if (x_fpsimd_preserve_current_state) x_fpsimd_preserve_current_state();
                memcpy(&new_fp_state, &current->thread.uw.fpsimd_state, sizeof(struct user_fpsimd_state));
...
static int shadow_step_handler(struct pt_regs *regs, hook_esr_t esr)
{
    ...
    spin_lock_irqsave(&shadow_page_lock, flags);
    list_for_each_entry(entry, &shadow_page_list, list) {
        if (entry->mm != current->mm || !atomic_read(&entry->step_pending)) continue;
        atomic_set(&entry->step_pending, 0);
        handled = true;
        spin_unlock_irqrestore(&shadow_page_lock, flags);

        patch_insn_current(entry->target_vaddr, SHADOW_BRK_INSN);
        if (x_user_disable_single_step) x_user_disable_single_step(current);
        return DBG_HOOK_HANDLED;
    }
    spin_unlock_irqrestore(&shadow_page_lock, flags);
...
int add_shadow(struct shadow_request *req)
{
    ...
    ret = patch_insn_user(task, target, SHADOW_BRK_INSN, &orig_insn, true);
    if (ret) { mmput(mm); put_task_struct(task); return ret; }

    entry = kzalloc(sizeof(*entry), GFP_KERNEL);
    if (!entry) {
        patch_insn_user(task, target, orig_insn, NULL, true);
        mmput(mm); put_task_struct(task); return -ENOMEM;
    }

    entry->vaddr = vaddr; entry->target_vaddr = target;
...
int shadow_fault_init_dynamic(struct shadow_sym_request *syms)
{
    ...
    x_user_disable_single_step = (void *)syms->p_user_disable_single_step;
    x_fpsimd_preserve_current_state = (void *)syms->p_fpsimd_preserve_current_state;
